...
void c_look(struct Request requests[], int N, int R, int H, int direction) {
    int current_time = 0;
    int total_seek_time = 0;
    int total_completion_time = 0;
    int head = H;
    ...
    while (requests_served_count < R) {
        ...
        if (next_request_index == -1) {
            ...
            // Requests exist but in the wrong direction, so wrap around to the opposite end of the disk (C-LOOK)
            int target_track = -1;
            for (int i = 0; i < R; i++) {
                if (!requests[i].served && requests[i].arrival_time <= current_time) {
                    if (direction == UP) {
                        if (target_track == -1 || requests[i].track < target_track) {
                            target_track = requests[i].track; // Wrap around to the lowest track in the current direction
                        }
                    } else {
                        if (target_track == -1 || requests[i].track > target_track) {
                            target_track = requests[i].track; // Wrap around to the highest track in the current direction
                        }
                    }
                }
            }
            int seek_distance = abs(head - target_track); // Calculate the seek distance from the current head position to the target track
            total_seek_time += seek_distance; // Accumulate the total seek time for moving to the target track
            current_time += seek_distance; // Time taken to move the head to the target track
            head = target_track; // Update the head position to the target track
            continue; // Check for requests again at the new current time
        }
        // Serve the next request
        int target_track = requests[next_request_index].track;
...
void main() {
    ...
    if (strcmp(algorithm, "SCAN") == 0 || strcmp(algorithm, "C-SCAN") == 0 || strcmp(algorithm, "LOOK") == 0 || strcmp(algorithm, "C-LOOK") == 0) {
    ...
    if (strcmp(algorithm, "FCFS") == 0) {
        ...
        c_look(requests, N, R, H, direction);
    } 
    else {
